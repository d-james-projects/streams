...
#ifndef BEJWRMUWYKY5JTUYPBAATWWPI_H_
#define BEJWRMUWYKY5JTUYPBAATWWPI_H_
...
#define SELF BeJwrMUwyKy5JTUypBAATwwPI
...
public:
    ...
    typedef SPL::boolean steady_type;

    enum { num_attributes = 1 } ;
    
    SELF() : Tuple(), steady_() {}
    SELF(const Self & ot) : Tuple(), steady_(ot.steady_) 
      { constructPayload(ot); }
    SELF(const steady_type & _steady) : Tuple(), steady_(_steady) { }
    ...
    steady_type & get_steady() { return steady_; }
    const steady_type & get_steady() const { return steady_; }
    void set_steady(const steady_type & _steady) { steady_ = _steady; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<steady_type >()(steady_);
        return s;
    }
    
    size_t getSerializedSize() const
    {
        size_t size = sizeof(SPL::boolean);
...
#undef SELF
#endif // BEJWRMUWYKY5JTUYPBAATWWPI_H_ 
