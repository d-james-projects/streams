...
#include "BeJwrMSw2NClKLPdPKk4tKkssyczPAwBFJwdU.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<rstring rawObservation>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("rawObservation");
        TypeOffset t(MY_OFFSETOF(rawObservation_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "rawObservation") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, rawObservation_);
   else
     istr >> rawObservation_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, rawObservation_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{'
         << "rawObservation=" << get_rawObservation()  
         << '}';
}
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "rawObservation=", get_rawObservation()) ;
    ostr << '}';
}

SELF& SELF::clear()
{
    SPL::rstring().swap(get_rawObservation());
