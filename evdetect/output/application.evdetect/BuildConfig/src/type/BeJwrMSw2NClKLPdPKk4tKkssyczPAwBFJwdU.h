...
#ifndef BEJWRMSW2NCLKLPDPKK4TKKSSYCZPAWBFJWDU_H_
#define BEJWRMSW2NCLKLPDPKK4TKKSSYCZPAWBFJWDU_H_
...
#define SELF BeJwrMSw2NClKLPdPKk4tKkssyczPAwBFJwdU
...
public:
    ...
    typedef SPL::rstring rawObservation_type;

    enum { num_attributes = 1 } ;
    
    SELF() : Tuple(), rawObservation_() {}
    SELF(const Self & ot) : Tuple(), rawObservation_(ot.rawObservation_) 
      { constructPayload(ot); }
    SELF(const rawObservation_type & _rawObservation) : Tuple(), rawObservation_(_rawObservation) { }
    ...
    rawObservation_type & get_rawObservation() { return rawObservation_; }
    const rawObservation_type & get_rawObservation() const { return rawObservation_; }
    void set_rawObservation(const rawObservation_type & _rawObservation) { rawObservation_ = _rawObservation; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<rawObservation_type >()(rawObservation_);
        return s;
    }
    ...
    {
        size_t size = 0;
           size += rawObservation_.getSerializedSize();
    ...
    { 
        rawObservation_ = ot.rawObservation_; 
        assignPayload(ot);
        return *this; 
    }
    ...
    void swap(SELF & ot) 
    { 
        std::swap(rawObservation_, ot.rawObservation_);
...
private:
    ...
    rawObservation_type rawObservation_;
...
#undef SELF
#endif // BEJWRMSW2NCLKLPDPKK4TKKSSYCZPAWBFJWDU_H_ 
