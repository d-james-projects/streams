// t3b4riseb4dropb4deep


#ifndef BEJWRMU4YKCOSTK0YSSNKLWCSQAKFAEYGADW_H_
#define BEJWRMU4YKCOSTK0YSSNKLWCSQAKFAEYGADW_H_
...
#define SELF BeJwrMU4yKcosTk0ySSnKLwCSqakFAEyGAdw
...
public:
    ...
    typedef SPL::boolean rise_type;
    typedef SPL::boolean drop_type;
    typedef SPL::boolean deep_type;

    enum { num_attributes = 3 } ;
    
    SELF() : Tuple(), rise_(), drop_(), deep_() {}
    SELF(const Self & ot) : Tuple(), rise_(ot.rise_), drop_(ot.drop_), deep_(ot.deep_) 
      { constructPayload(ot); }
    SELF(const rise_type & _rise, const drop_type & _drop, const deep_type & _deep) : Tuple(), rise_(_rise), drop_(_drop), deep_(_deep) { }
    ...
    rise_type & get_rise() { return rise_; }
    const rise_type & get_rise() const { return rise_; }
    void set_rise(const rise_type & _rise) { rise_ = _rise; }
    drop_type & get_drop() { return drop_; }
    const drop_type & get_drop() const { return drop_; }
    void set_drop(const drop_type & _drop) { drop_ = _drop; }
    deep_type & get_deep() { return deep_; }
    const deep_type & get_deep() const { return deep_; }
    void set_deep(const deep_type & _deep) { deep_ = _deep; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<rise_type >()(rise_);
        s = 37 * s + std::tr1::hash<drop_type >()(drop_);
        s = 37 * s + std::tr1::hash<deep_type >()(deep_);
        return s;
    }
    
    size_t getSerializedSize() const
    {
        size_t size = sizeof(SPL::boolean)+sizeof(SPL::boolean)+sizeof(SPL::boolean);
    ...
    void swap(SELF & ot) 
    { 
        std::swap(rise_, ot.rise_);
        std::swap(drop_, ot.drop_);
        std::swap(deep_, ot.deep_);
...
#undef SELF
#endif // BEJWRMU4YKCOSTK0YSSNKLWCSQAKFAEYGADW_H_ 
