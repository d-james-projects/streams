...
#include "BeJwrMUwyKy5JTUypBAATwwPI.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<boolean steady>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("steady");
        TypeOffset t(MY_OFFSETOF(steady_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "steady") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, steady_);
   else
     istr >> steady_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, steady_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "steady=", get_steady()) ;
    ostr << '}';
}
