...
#ifndef BEJWRMSW2NMGQZS8LLINKZESHACRHAWR_H_
#define BEJWRMSW2NMGQZS8LLINKZESHACRHAWR_H_
...
#define SELF BeJwrMSw2NMgqzs8LLinKzEsHACrhAWr
...
public:
    ...
    typedef SPL::rstring jsonString_type;

    enum { num_attributes = 1 } ;
    
    SELF() : Tuple(), jsonString_() {}
    SELF(const Self & ot) : Tuple(), jsonString_(ot.jsonString_) 
      { constructPayload(ot); }
    SELF(const jsonString_type & _jsonString) : Tuple(), jsonString_(_jsonString) { }
    ...
    jsonString_type & get_jsonString() { return jsonString_; }
    const jsonString_type & get_jsonString() const { return jsonString_; }
    void set_jsonString(const jsonString_type & _jsonString) { jsonString_ = _jsonString; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<jsonString_type >()(jsonString_);
        return s;
    }
    ...
    {
        size_t size = 0;
           size += jsonString_.getSerializedSize();
    ...
    void swap(SELF & ot) 
    { 
        std::swap(jsonString_, ot.jsonString_);
...
#undef SELF
#endif // BEJWRMSW2NMGQZS8LLINKZESHACRHAWR_H_ 
